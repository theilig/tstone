...
trait ItemTable {
   ...
   *  @param itemTraits Database column item_traits SqlType(VARCHAR), Length(100,true) */
  case class ItemRow(cardId: Int, light: Int, cost: Int, goldValue: Int, victoryPoints: Int, itemTraits: String)
  /** GetResult implicit for fetching ItemRow objects using plain SQL queries */
  implicit def GetResultItemRow(implicit e0: GR[Int], e1: GR[String]): GR[ItemRow] = GR{
    prs => import prs._
    ItemRow.tupled((<<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[String]))
  }
  ...
  class Item(_tableTag: Tag) extends profile.api.Table[ItemRow](_tableTag, Some("TStone"), "Item") {
    def * = (cardId, light, cost, goldValue, victoryPoints, itemTraits) <> (ItemRow.tupled, ItemRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(cardId), Rep.Some(light), Rep.Some(cost), Rep.Some(goldValue), Rep.Some(victoryPoints), Rep.Some(itemTraits))).shaped.<>({r=>import r._; _1.map(_=> ItemRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column item_traits SqlType(VARCHAR), Length(100,true) */
    val itemTraits: Rep[String] = column[String]("item_traits", O.Length(100,varying=true))
  }
