...
trait VillageEffectTable {
  ...
  implicit def GetResultVillageEffectRow(implicit e0: GR[Int], e1: GR[Option[String]], e2: GR[Boolean], e3: GR[Option[Int]]): GR[VillageEffectRow] = GR{
    prs => import prs._
    VillageEffectRow.tupled((<<[Int], <<[Int], <<?[String], <<?[String], <<[Boolean], <<?[String], <<?[Int], <<?[String]))
  }
  ...
  class VillageEffect(_tableTag: Tag) extends profile.api.Table[VillageEffectRow](_tableTag, Some("TStone"), "VillageEffect") {
    def * = (effectId, cardId, effect, needType, repeatable, operation, modifierAmount, attributeModified) <> (VillageEffectRow.tupled, VillageEffectRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(effectId), Rep.Some(cardId), effect, needType, Rep.Some(repeatable), operation, modifierAmount, attributeModified)).shaped.<>({r=>import r._; _1.map(_=> VillageEffectRow.tupled((_1.get, _2.get, _3, _4, _5.get, _6, _7, _8)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column operation SqlType(ENUM), Length(4,false), Default(None) */
