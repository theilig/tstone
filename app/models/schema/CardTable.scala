...
trait CardTable {
  ...
  case class CardRow(cardId: Int, image: String, name: String, frequency: Int)
  ...
  implicit def GetResultCardRow(implicit e0: GR[Int], e1: GR[String]): GR[CardRow] = GR{
    prs => import prs._
    CardRow.tupled((<<[Int], <<[String], <<[String], <<[Int]))
  }
  ...
  class Card(_tableTag: Tag) extends profile.api.Table[CardRow](_tableTag, Some("TStone"), "Card") {
    def * = (cardId, image, name, frequency) <> (CardRow.tupled, CardRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(cardId), Rep.Some(image), Rep.Some(name), Rep.Some(frequency))).shaped.<>({r=>import r._; _1.map(_=> CardRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
