...
// AUTO-GENERATED Slick data model for table BattleEffect
trait BattleEffectTable {
  ...
  /** Entity class storing rows of table BattleEffect
  ...
  case class BattleEffectRow(effectId: Int, cardId: Int, effect: Option[String] = None, needType: Option[String] = None, repeatable: Boolean, operation: Option[String] = None, modifierAmount: Option[Int] = None, attributeModified: Option[String] = None)
  /** GetResult implicit for fetching BattleEffectRow objects using plain SQL queries */
  implicit def GetResultBattleEffectRow(implicit e0: GR[Int], e1: GR[Option[String]], e2: GR[Boolean], e3: GR[Option[Int]]): GR[BattleEffectRow] = GR{
    prs => import prs._
    BattleEffectRow.tupled((<<[Int], <<[Int], <<?[String], <<?[String], <<[Boolean], <<?[String], <<?[Int], <<?[String]))
  }
  ...
  class BattleEffect(_tableTag: Tag) extends profile.api.Table[BattleEffectRow](_tableTag, Some("TStone"), "BattleEffect") {
    def * = (effectId, cardId, effect, needType, repeatable, operation, modifierAmount, attributeModified) <> (BattleEffectRow.tupled, BattleEffectRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(effectId), Rep.Some(cardId), effect, needType, Rep.Some(repeatable), operation, modifierAmount, attributeModified)).shaped.<>({r=>import r._; _1.map(_=> BattleEffectRow.tupled((_1.get, _2.get, _3, _4, _5.get, _6, _7, _8)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  /** Collection-like TableQuery object for table BattleEffect */
  lazy val BattleEffect = new TableQuery(tag => new BattleEffect(tag))
}
