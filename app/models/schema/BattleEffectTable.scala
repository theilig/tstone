...
trait BattleEffectTable {
  ...
  case class BattleEffectRow(effectId: Int, cardId: Int, effect: String)
  /** GetResult implicit for fetching BattleEffectRow objects using plain SQL queries */
  implicit def GetResultBattleEffectRow(implicit e0: GR[Int], e1: GR[String]): GR[BattleEffectRow] = GR{
    prs => import prs._
